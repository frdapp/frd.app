    ...
    <VBox style="width:100%">
        ...
        <div style="margin-top:10px;padding:0px;width:100%">
            <Table :items="items" @sort-change="searchSort" style="width: 100%"
                ...
                <Column prop="identify" :label="$t('Identify')" width="100"> </Column>
                <Column prop="subject" :label="$t('Email Subject')" width="100"> </Column>
                <Column prop="content" :label="$t('Email Content')" width="100"> </Column>
                ...
                <Column align="center" :label="$t('Operate')" width="300">
                    <template #default="scope">
                        <a :href="'/manage/email_template/update?id=' + scope.item.id" style="margin-left:10px"
...
export default {
    ...
    methods: {
        ...
        openDeleteDialog: async function (id) {
            ...
            if (result) {
                var response = await this.$api.manage_email_template_delete({ "id": id })
                if (response != false) {
                    this.search()
                }
            }
        ...
        openAddDialog: function () {
            this.$router.push({
                path: "/manage/email_template/create",
                query: {}
            });
            return;
        },
        ...
        search: async function () {
            var response = await this.$api.manage_email_template_list(this.query)
