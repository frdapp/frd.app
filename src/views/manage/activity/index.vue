    ...
    -->
        ...
        <div style="margin-top:10px;padding:0px;width:100%">
            <Table :items="items" @sort-change="searchSort" style="width: 100%"
                ...
                <Column prop="activity_type" :label="$t('Activity Type')" width="100"> </Column>
                <Column prop="on_event" :label="$t('On Event')" width="100"> </Column>
                <Column prop="title" :label="$t('Title')">
                </Column>
                <Column prop="description" :label="$t('Description')" width="100"> </Column>
                <Column prop="params" :label="$t('Params')" width="100"> 
                    <template #default="scope">
                        <pre>
                            {{ scope.item.params }}
                        </pre>
...
export default {
    ...
    methods: {
        ...
        openDeleteDialog: async function (id) {
            ...
            if (result) {
                var response = await this.$api.manage_activity_delete({ "id": id })
                if (response != false) {
                    this.search()
                }
            }
        ...
        search: async function () {
            var response = await this.$api.manage_activity_list(this.query)
