    ...
    -->
        ...
        <div style="margin-top:10px;padding:0px;width:100%">
            <Table :items="items" @sort-change="searchSort" style="width: 100%"
                ...
                <Column prop="refund_no" :label="$t('Refund No')">
                </Column>
                <Column prop="reason" :label="$t('Reason')" width="100"> </Column>
                ...
                <Column align="center" :label="$t('Operate')" width="300">
                    <template #default="scope">
                        <a :href="'/manage/order_refund/update?id=' + scope.item.id" style="margin-left:10px"
...
export default {
    ...
    methods: {
        ...
        openDeleteDialog: async function (id) {
            ...
            if (result) {
                var response = await this.$api.api_post("/api/manage/order_refund/delete",{ "id": id })
                if (response != false) {
                    this.search()
                }
            }
        ...
        openAddDialog: function () {
            this.$router.push({
                path: "/manage/order_refund/create",
                query: {}
            });
            return;
        },
        ...
        search: async function () {
            var response = await this.$api.api_get("/api/manage/order_refund/list",this.query)
